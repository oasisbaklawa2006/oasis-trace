...
export default function Printers() {
  ...
  return (
    <div>
      <PageHeader eyebrow="Setup" title="Printer Management" description="TSC TE244 / TE200 / Zebra-compatible. TSPL & ZPL command generation, calibration, test print, persistent printer profiles." />
...
function PrinterCard({ printer, onEdit }: { printer: PrinterRow; onEdit: () => void }) {
  ...
  function testPrint() {
    ...
    try {
      ...
      toast.error("Clipboard unavailable", { description: "Open browser print dialog instead." });
    }
  }
  return (
    <div className="ols-card p-5">
      ...
      <div className="mt-4 flex gap-2">
        <Button size="sm" variant="outline" className="flex-1" onClick={onEdit}>
          <Wrench size={14} className="mr-1" /> Calibrate
        </Button>
        <Button size="sm" className="flex-1 bg-gradient-primary text-primary-foreground" onClick={testPrint}>
          Test Print
        </Button>
      </div>
    </div>
      ...
      <div className="h-full w-full max-w-md overflow-y-auto bg-card p-6 shadow-elevated" onClick={e => e.stopPropagation()}>
        ...
        <div className="mt-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="mb-1.5 block text-xs">Label gap (mm)</Label>
              <Input value={gap} onChange={e => setGap(e.target.value)} type="number" step="0.5" min={0} />
            </div>
            <div>
              <Label className="mb-1.5 block text-xs">Black-mark offset (mm)</Label>
              <Input value={bm} onChange={e => setBm(e.target.value)} type="number" step="0.5" min={0} placeholder="optional" />
            </div>
            <div>
              <Label className="mb-1.5 block text-xs">DPI</Label>
              <Input value={dpi} onChange={e => setDpi(e.target.value)} type="number" />
            </div>
          </div>
        </div>
